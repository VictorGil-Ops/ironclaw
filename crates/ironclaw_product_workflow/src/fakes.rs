...
impl FakeInboundTurnService {
    ...
    pub fn program_outcome(&self, outcome: InboundTurnOutcome) {
        let mut state = self.state.lock().expect("fake inbound turn state lock poisoned"); // safety: test-support fake
    ...
    pub fn accepted_envelopes(&self) -> Vec<ProductInboundEnvelope> {
        let state = self.state.lock().expect("fake inbound turn state lock poisoned"); // safety: test-support fake
...
impl InboundTurnService for FakeInboundTurnService {
    async fn accept_user_message(
        ...
        .map_err(|e: String| ProductWorkflowError::TurnSubmissionRejected {
            reason: e,
        })?;
